    ...
    def to_canonical(self) -> dict[str, Any]:
        return {
            ...
            for field in (
                ...
                "positive_precision",
                "positive_recall",
                "ambiguous_forced_negative",
                "expected_no_link_preserved",
            )
        }
...
def _norm(value: str | None) -> str:
    return (
        ""
        if value is None
        else _SPACE.sub(" ", unicodedata.normalize("NFC", value).casefold().strip())
    )
...
def _native_signal(
    ...
    if matches and conflicts:
        return EntityDecision.AMBIGUOUS, (
            "contradictory-native-identifiers",
            *sorted(matches | conflicts),
        )
    if matches:
        return EntityDecision.SAME_ENTITY, ("shared-provider-native-identifier", *sorted(matches))
    if conflicts:
        return EntityDecision.DIFFERENT_ENTITY, (
            "conflicting-provider-native-identifier",
            *sorted(conflicts),
        )
    return None
...
def _relations(
    item: LabObservation, target: str | None, as_of: datetime
) -> tuple[LabRelation, ...]:
    if target is None:
        return ()
    return tuple(
        r for r in item.relations if r.target_external_ref == target and r.eligible_at(as_of)
    )
...
def assess_entity(
    ...
    if candidate_version in {"explicit-native-id-v0", "transparent-entity-hybrid-v0"}:
        ...
        if candidate_version == "explicit-native-id-v0":
            return Assessment(
                candidate_version, EntityDecision.AMBIGUOUS, ("no-shared-native-identity",)
            )
    ...
    if candidate_version == "artifact-coordinate-v0":
        ...
        return (
            ...
            if signal is not None
                ...
                candidate_version, EntityDecision.AMBIGUOUS, ("artifact-coordinate-insufficient",)
            )
        )
    ...
    if candidate_version == "transparent-entity-hybrid-v0":
        ...
        if any(r.relation_type in _ALIASES for r in relations):
            return Assessment(
                ...
                ("point-in-time-explicit-alias-or-rename",),
            )
    ...
    if same_target:
        return Assessment(
            candidate_version,
            EntityDecision.SAME_ENTITY,
            ("same-canonical-target", "matching-entity-type-name"),
        )

    reason = (
        "canonical-target-insufficient"
        if candidate_version == "canonical-entity-target-v0"
        else "insufficient-stable-identity-evidence"
    )
    return Assessment(candidate_version, EntityDecision.AMBIGUOUS, (reason,))


def _direct_relation(
    left: LabObservation, right: LabObservation, as_of: datetime
) -> LabRelation | None:
    values = (
        *_relations(left, right.canonical_url, as_of),
        *_relations(right, left.canonical_url, as_of),
...
def assess_provenance(
    ...
    if candidate_version in {"explicit-reference-v0", "transparent-provenance-hybrid-v0"}:
        ...
        if relation is not None:
            return Assessment(
                candidate_version,
                ProvenanceDecision.DIRECT_DERIVATIVE,
        ...
        if candidate_version == "explicit-reference-v0":
            return Assessment(
                candidate_version,
                ProvenanceDecision.NO_LINK_EVIDENCE,
                ("no-qualifying-explicit-derivation",),
                "NO_LINK_EVIDENCE is not factual independence",
            )

    if candidate_version in {
        "exact-content-common-upstream-v0",
        "transparent-provenance-hybrid-v0",
    } and _mirror(left, right):
        return Assessment(
            candidate_version,
            ProvenanceDecision.SHARED_UPSTREAM_POSSIBLE,
...
def _relation(document: dict[str, Any]) -> LabRelation:
    ...
    return LabRelation(
        ...
        str(document["target_external_ref"]),
        str(document["authority"]),
        None if created is None else _time(str(created)),
    )
...
def _observation(document: dict[str, Any]) -> LabObservation:
    return LabObservation(
        ...
        canonical_url=None
        if document.get("canonical_url") is None
...
def _metrics(cases: tuple[LabCase, ...], candidate: str, entity: bool) -> CandidateMetrics:
    if entity:
        ...
        predicted = [
            assess_entity(candidate, x.left, x.right, as_of=x.as_of).decision for x in cases
        ]
        positive: EntityDecision | ProvenanceDecision = EntityDecision.SAME_ENTITY
        forced = sum(
            e is EntityDecision.AMBIGUOUS and p is EntityDecision.DIFFERENT_ENTITY
        ...
        predicted = [
            assess_provenance(candidate, x.left, x.right, as_of=x.as_of).decision for x in cases
        ]
        positive = ProvenanceDecision.DIRECT_DERIVATIVE
        forced = 0
        preserved = sum(
            e is ProvenanceDecision.NO_LINK_EVIDENCE and p is ProvenanceDecision.NO_LINK_EVIDENCE
            for e, p in zip(expected, predicted, strict=True)
        )
    positive_expected = sum(x is positive for x in expected)
    positive_predictions = sum(x is positive for x in predicted)
    true_positives = sum(
        e is positive and p is positive for e, p in zip(expected, predicted, strict=True)
    )
    return CandidateMetrics(
        candidate,
        len(cases),
        sum(e is p for e, p in zip(expected, predicted, strict=True)),
        positive_expected,
        positive_predictions,
        true_positives,
        positive_predictions - true_positives,
        None if positive_predictions == 0 else _ratio(true_positives, positive_predictions),
        _ratio(true_positives, positive_expected),
        forced,
        preserved,
    )


def evaluate_entity_candidate(
    cases: tuple[LabCase, ...], candidate_version: str
) -> CandidateMetrics:
    return _metrics(cases, candidate_version, True)


def evaluate_provenance_candidate(
    cases: tuple[LabCase, ...], candidate_version: str
) -> CandidateMetrics:
...
def _select(metrics: tuple[CandidateMetrics, ...], entity: bool) -> str:
    survivors = [
        x for x in metrics if x.false_positives == 0 and x.positive_precision == "1.000000"
    ]
    if not survivors:
    ...
    complexity = (
        ...
        if entity
            ...
            "explicit-reference-v0": 1,
            "exact-content-common-upstream-v0": 1,
            "transparent-provenance-hybrid-v0": 2,
        }
    )
    survivors.sort(
        key=lambda x: (
...
def build_selection_report(
    cases: tuple[LabCase, ...], *, corpus_digest_value: str
) -> LabSelectionReport:
    ...
    return LabSelectionReport(
        corpus_digest_value, entity, provenance, _select(entity, True), _select(provenance, False)
    )
