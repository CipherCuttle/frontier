...
def validate_case_shape(cases: list[dict[str, object]], *, minimum: int) -> None:
    ...
    for index, case in enumerate(cases):
        ...
        require(
            isinstance(assertions, dict) and assertions, f"{case_id} must have non-empty assertions"
        )
        ids.append(case_id)
...
def validate_acquisition_corpus() -> list[dict[str, object]]:
    ...
    require(
        REQUIRED_SOURCES <= sources, f"missing source lanes: {sorted(REQUIRED_SOURCES - sources)}"
    )
    ...
    require(
        xxe_assertions.get("external_entity_resolutions") == 0,
        "XXE fixture must permit zero entity resolution",
    )
    require(
        xxe_assertions.get("network_side_effects") == 0,
        "XXE fixture must permit zero network side effects",
    )
    ...
    require(
        redirect_assertions.get("forbidden_connections") == 0,
        "private-IP redirect must never connect",
    )
    ...
    require(
        window_gap.get("expected") == "DEGRADE", "finite feed-window gap must degrade completeness"
    )
    require(
        window_assertions.get("transport_health") == "OK",
        "finite feed-window fixture must prove transport can stay healthy",
    )
    require(
        window_assertions.get("completeness_health") == "DEGRADED",
        "finite feed-window fixture must degrade completeness",
    )
    ...
    require(
        backfill_assertions.get("live_alert_eligible") is False,
        "backfill must not become a live alert solely because it was ingested now",
    )
    ...
    require(
        hn_assertions.get("attention_observations") == 2,
        "HN duplicate-root fixture must preserve two attention observations",
    )
    require(
        hn_assertions.get("external_factual_roots_max") == 1,
        "HN duplicate-root fixture must not manufacture corroboration",
    )
    ...
    require(
        gdelt_assertions.get("discovery_rows") == 6,
        "GDELT syndication fixture must preserve discovery propagation",
    )
    require(
        gdelt_assertions.get("independent_factual_roots_max") == 1,
        "GDELT syndication fixture must not manufacture six roots",
    )
    ...
    require(
        malformed_assertions.get("default_malformed_time_to_now") is False,
        "malformed source times must never silently become now",
    )
...
def validate_transport_pack() -> list[dict[str, object]]:
    ...
    require(
        rebinding_assertions.get("connect_target_revalidated") is True,
        "DNS rebinding must revalidate the actual connect target",
    )
    require(
        rebinding_assertions.get("forbidden_connections") == 0,
        "DNS rebinding must never reach private target",
    )
    ...
    require(
        credentials_assertions.get("forward_authorization_cross_origin") is False,
        "cross-origin redirect must not forward Authorization",
    )
    require(
        credentials_assertions.get("secret_leaks") == 0, "redirect fixture must leak zero secrets"
    )
    ...
    require(
        bomb_assertions.get("expanded_limit_enforced") is True,
        "expanded-byte limit must be enforced",
    )
    require(
        bomb_assertions.get("full_body_buffered") is False,
        "expansion bomb must not require full buffering",
    )
    ...
    require(
        length_assertions.get("stream_limit_enforced_independent_of_header") is True,
        "stream limits must not trust Content-Length",
    )
    ...
    require(
        mime_assertions.get("transport_health") == "OK",
        "MIME lie must prove transport can be healthy",
    )
    require(
        mime_assertions.get("schema_health") == "DEGRADED", "MIME lie must degrade schema health"
    )
    ...
    require(
        slow_assertions.get("partial_body_not_canonical") is True,
        "partial timed-out body must not become canonical",
    )
    ...
    require(
        retry_assertions.get("retry_after_respected") is True, "429 Retry-After must be respected"
    )
    require(
        retry_assertions.get("immediate_retry") is False, "429 must not trigger immediate retry"
    )
    ...
    require(
        absurd_assertions.get("provider_delay_not_trusted_unbounded") is True,
        "Retry-After must not grant unbounded scheduling authority",
    )
    require(
        absurd_assertions.get("retry_budget_bounded") is True, "retry budget must remain bounded"
    )
    ...
    require(
        missing_cache.get("expected") == "FAIL_CLOSED", "304 without cached entity must fail closed"
    )
    require(
        missing_cache_assertions.get("invent_content_from_304") is False,
        "304 must never manufacture missing content",
    )
    ...
    require(
        stale_assertions.get("validator_inconsistency_detected") is True,
        "same validator with changed bytes must be visible",
    )
    ...
    require(
        recovery_assertions.get("recovered_after_gap") is True,
        "recovery backlog must carry gap context",
    )
    require(
        recovery_assertions.get("delivery_burst_is_not_world_burst") is True,
        "delivery burst must not equal world-event burst",
    )
    require(
        recovery_assertions.get("live_breakout_due_only_to_recovery") is False,
        "recovery alone must not manufacture a live breakout",
    )
...
def normalization_case_ids() -> list[str]:
    ...
    for index, case in enumerate(raw_cases):
        ...
        require(
            ...
            f"normalization/provenance case {index} missing id",
        )
        ids.append(case_id)
    return ids
...
def main() -> None:
    ...
    require(
        len(all_ids) == len(set(all_ids)), "fixture case ids must be unique across all three packs"
    )
    print(
        "validated "
